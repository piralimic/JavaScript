...
(() => {
  ...
  const mauvaisesPropositions = [];

  function guessLetter() {
    let propositionJoueur = prompt("Proposez une lettre : ");
    const lettresSecretesTotales = lettresSecretes.length;

    if (listePropositions.includes(propositionJoueur)) {
      alert(`Vous avez déjà proposé la lettre : ${propositionJoueur}`)
    } else {
      listePropositions.push(propositionJoueur);
      ...
      if (!lettresSecretes.includes(propositionJoueur)) {
        mauvaisesPropositions.push(propositionJoueur);
      }
    }


    document.getElementById("demo").innerText = lettresDecouvertes.join(" ");
    document.getElementById("mode").innerText = mauvaisesPropositions.join();
